...
commonPlugin.install = function (Vue) {
  ...
  Vue.directive("limitWord", {
    inserted: function (el, binding, vnode) {
      ...
      insertEl.style.color = targetEl.value.length > maxLength ? "red" : "#cbcbcb";
    ...
    bind: (el) => {
      ...
      title.onmousedown = function ($ev) {
        ...
        let translateX = oriTranslate
          ? oriTranslate.split(",")[0].slice(10, oriTranslate.split(",")[0].length - 2)
          : 0;
        let translateY = oriTranslate
          ? oriTranslate.split(",")[1].slice(0, oriTranslate.split(",")[1].length - 3)
          : 0;
